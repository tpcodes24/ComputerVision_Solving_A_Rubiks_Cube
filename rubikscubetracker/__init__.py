...
class RubiksVideo(RubiksOpenCV):
    ...
    def __init__(self, webcam,index=0, name=None, debug=False):
        ...
        while True:
            ...
            if self.save_colors and self.size and len(self.data.keys()) == (self.size * self.size):
                ...
                if self.name == 'F':
                    ...
                    cmd = ['py','rubiks-color-resolver.py', '--json', '--filename', '/tmp/webcam.json']
                    log.info(' '.join(cmd))
                    ...
                    # Run the solver
                        ...
                        cmd = 'python rubiks-cube-solver.py --colormap "%s" --state %s' % (colormap, kociemba_string)
                        log.info(cmd)
